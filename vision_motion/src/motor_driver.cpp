...
bool MotorCommand::writePosition(const int64_t value[6]) {
    ...
    dxl_addparam_result = groupSyncWritePosition_->addParam(DXL_M_LEFT_ID, (uint8_t*)&data_byte);
    if (dxl_addparam_result != true)
        return false;
    ...
    dxl_addparam_result = groupSyncWritePosition_->addParam(DXL_M_RIGHT_ID, (uint8_t*)&data_byte);
    if (dxl_addparam_result != true)
        return false;

    data_byte[0] = DXL_LOBYTE(DXL_LOWORD(value[4]));
    data_byte[1] = DXL_HIBYTE(DXL_LOWORD(value[4]));
    data_byte[2] = DXL_LOBYTE(DXL_HIWORD(value[4]));
    data_byte[3] = DXL_HIBYTE(DXL_HIWORD(value[4]));
    dxl_addparam_result = groupSyncWritePosition_->addParam(DXL_R_LEFT_ID, (uint8_t*)&data_byte);
    if (dxl_addparam_result != true)
        return false;

    data_byte[0] = DXL_LOBYTE(DXL_LOWORD(value[5]));
    data_byte[1] = DXL_HIBYTE(DXL_LOWORD(value[5]));
    data_byte[2] = DXL_LOBYTE(DXL_HIWORD(value[5]));
    data_byte[3] = DXL_HIBYTE(DXL_HIWORD(value[5]));
