...
public:
    DDSM_Twist() : Node("DDSM115"), port1(0), port2(2), port3(4), port4(6){
        declare_parameter("WO1", 0.44);
        declare_parameter("WO2", 0.0);
        declare_parameter("WO3", 0.46);
        declare_parameter("AW", 0.55);
    ...
    void twist_cb(const geometry_msgs::msg::Twist msg) {
        ...
        if (std::abs(v) < 0.01 && std::abs(omega) > 0.01) { // rot. in place
            ...
            motor_rpm_[1] = -1 * sqrt(pow(wheelOffset1_, 2) + pow(axleWidth_ / 2, 2)) * omega / (wheelDiameter_ / 2) * 30 / PI;
            motor_rpm_[2] = -1 * sqrt(pow(wheelOffset2_, 2) + pow(axleWidth_ / 2, 2)) * omega / (wheelDiameter_ / 2) * 30 / PI;
            motor_rpm_[3] = -1 * sqrt(pow(wheelOffset2_, 2) + pow(axleWidth_ / 2, 2)) * omega / (wheelDiameter_ / 2) * 30 / PI;
            motor_rpm_[4] = -1 * sqrt(pow(wheelOffset3_, 2) + pow(axleWidth_ / 2, 2)) * omega / (wheelDiameter_ / 2) * 30 / PI;
            motor_rpm_[5] = -1 * sqrt(pow(wheelOffset3_, 2) + pow(axleWidth_ / 2, 2)) * omega / (wheelDiameter_ / 2) * 30 / PI;
        }
        else if (omega < -0.01) {
