...
ZLAC::ZLAC(std::string portname, int baudrate, uint8_t ID) : port(io, portname) {
    this->ID = ID;
...
uint8_t ZLAC::read_hex(uint8_t num_bytes) {  //crc err check (err = 1)
    ...
    boost::asio::read(port, buf.prepare(num_bytes));
    buf.commit(num_bytes);
    std::istream is(&buf);
    std::string line;
    std::getline(is, line);
